...
随着安卓开发市场涌入越来越多的开发者，对于应用分析和需求的规划也变得更加重要。显而易见的是，应用的测试和调整十分依赖于用户直接反馈、关注群体以及用户的评价。Google Analystics可以作为开发过程的一个环节集成到应用中去找出应用的不足和用户的痛点。
...
注意上面例子里的1.4.0-beta3版本只是写书时的版本号，你可以更新到最新的版本使用一些新的功能和特性。同时还要在app module 下的build.gradle文件中做一些添加，增加的内容如下：
